        ...
        for(i=0;i<nfds;i++)
        {
           ...
           // 客户端有数据发送过来
            else
            {
                ...
                len=recv(c_fd,buf,BUFFER_SIZE,0);
                if(len<0)
                {
                ...
                if(c_fd){
                    int send_ret;
                    printf("receive from client:%s, c_fd is %d\n",buf, c_fd);
                    send_ret = send(c_fd,"I have received your message.",30, MSG_NOSIGNAL);
                    if(-1 == send_ret){
                        /*if(epoll_ctl(epoll_fd,EPOLL_CTL_DEL, c_fd, &ev)==-1)
                        {
                            perror("epll_ctl:client_sockfd delete failed");
                            exit(EXIT_FAILURE);
                        }*/
                    close(c_fd);
		    }
